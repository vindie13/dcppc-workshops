...
## Load raw data
df <- read_csv("~/Downloads/tito-dcppc-june-data-commons-workshop-tickets-for-rayna-harris-gmail-com-1096288.csv")
...
df$`Which Team are you a member of?` <- ifelse(grepl("Data Stewards", df$`Which Team are you a member of?`), "Data Stewards",
                  ifelse(grepl("NIH", df$`Which Team are you a member of?`), "Hydrogen (NIH)",
...
```{r hackathon, echo = F, message = F, include=F}
# hackathon or no hackathon

names(df)[11]<-"Hackathon"

df$Hackathon <- ifelse(grepl("combination", df$Hackathon), "combination",
                  ifelse(grepl("Hackathon", df$Hackathon), "hackathon only",
                  ifelse(grepl("Workshop", df$Hackathon), "workshop only", NA)))

attendees <- df %>% 
  select(`Ticket Full Name`, `Team`, `Hackathon`, `Ticket Email`, `Have you registered for \"CollaborationFest\", too?`) 
attendees <- as.data.frame(attendees)

attendees$`Ticket Full Name` <- as.character(attendees$`Ticket Full Name`)
attendees <- attendees %>%  
  arrange(`Ticket Full Name`)
names(attendees)[4]<-"Email"
write.csv(attendees, "./data/2018-june-attendees.csv")

# using google sheets to read in collaboration fest data
gs_ls()
cofest <- gs_title("CoFest 2018 Participants")
participants <- gs_read(ss=cofest, ws = "Sheet1", skip = 2, check.names = F)
participants <- participants[c(2:4,18)] # manually selected for importance

fj <- full_join(attendees, participants, by = "Email")


onboarding <- gs_title("DCPPC all people Consortium responses to form")
onboarded <- gs_read(ss=onboarding, ws = "Form responses 1", check.names = F)
onboarded <- onboarded[c(2,4,3)] # manually selected for importance
names(onboarded)[2]<-"Email"

lg <- left_join(fj, onboarded, by = "Email")

write.csv(lg, "./data/2018-june-attendees-cofest.csv")
```
